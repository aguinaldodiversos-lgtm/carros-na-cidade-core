  ...
  it("returns payment summary for given period", async () => {
    vi.mocked(query).mockResolvedValueOnce({
      rows: [{
        ...
        rejected_count: 5,
        canceled_count: 5,
        total_approved_amount: 15000.00,
        total_pending_amount: 3000.00,
        plan_approved_count: 20,
        boost_approved_count: 10,
        plan_approved_amount: 10000.00,
        boost_approved_amount: 5000.00,
      }],
    });
    ...
    expect(result.total_approved_amount).toBe(15000.00);
    expect(result.plan_approved_amount).toBe(10000.00);
    expect(result.boost_approved_amount).toBe(5000.00);
  });
