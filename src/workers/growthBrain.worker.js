...
async function refreshAdMetrics() {
  try {
    ...
    // fallback se não suportar concurrently
...
async function upsertCityDominanceTop(limit = 50) {
  ...
  // Dominância = (leads*5) + (avg_ctr*100) + total_ads + demanda (se existir)
  await pool.query(
    `
...
async function upsertLearningModelTop(limit = 300) {
  ...
  // Aprende “model” por cidade: CTR e leads
  await pool.query(
    `
 ...
 */
async function enqueueJob(job_type, payload, priority = 3) {
  await pool.query(
    `
...
/**
 * Oferta de upgrade (consentimento): só cria notificação, NÃO altera plano.
 * Anti-spam: 7 dias.
 */
...
async function planCityActions(limit = 10) {
  ...
  for (const city of top.rows) {
    ...
    // Campanha automática (só enfileira ação, execução real depois)
    ...
    // Detecção de oportunidade: cidade com CTR bom e poucos anúncios
...
async function runOnce() {
  ...
  try {
    logger.info({
      message: "🧠 Growth Brain 2.0 iniciando...",
      instance,
    });
    ...
    // Sempre com consentimento: só notificação
    await enqueueUpgradeOffers();

    logger.info({
      message: "✅ Growth Brain 2.0 finalizado",
      instance,
    });
  } catch (err) {
    logger.error({
      message: "❌ Erro no Growth Brain 2.0",
      error: err.message,
      instance,
    });
  }
...
export function startGrowthBrainWorker() {
  runOnce();
  setInterval(runOnce, 6 * 60 * 60 * 1000); // 6h
}
