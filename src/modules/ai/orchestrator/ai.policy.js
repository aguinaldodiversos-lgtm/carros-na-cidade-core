...
export class AiPolicy {
  ...
  mode() {
    return String(process.env.AI_MODE || "hybrid"); // local | premium | hybrid
  }
  ...
  shouldCache(task) {
    // quase sempre sim; se tiver algo ultra-dinâmico, desligue aqui
    return true;
  }

  cacheKey({ task, input, context }) {
    // IMPORTANT: não cachear PII “crua”
    const pii = Boolean(context?.pii);
    const normalized = {
      task,
      // se for PII, não inclui o input integral (evita vazar em cache)
