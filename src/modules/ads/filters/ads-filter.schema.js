...
function optionalTrimmedString({ min = 1, max = 120, toLowerCase = false } = {}) {
  return z.preprocess(
    (value) => {
...
function optionalPositiveNumber({ min = 0, max = Number.MAX_SAFE_INTEGER } = {}) {
  return z.preprocess(
    (value) => normalizeNumber(value),
...
const TRANSMISSION_MAX_LENGTH = ADS_FILTER_LIMITS?.TRANSMISSION_MAX_LENGTH ?? 40;
...
export const AdsFilterSchema = z
  .object({
    q: optionalTrimmedString({ min: QUERY_MIN_LENGTH, max: QUERY_MAX_LENGTH }),
    brand: optionalTrimmedString({ min: 1, max: BRAND_MAX_LENGTH }),
    model: optionalTrimmedString({ min: 1, max: MODEL_MAX_LENGTH }),
    city: optionalTrimmedString({ min: 1, max: CITY_MAX_LENGTH }),
    city_slug: optionalTrimmedString({ min: 1, max: CITY_MAX_LENGTH, toLowerCase: true }),
    state: z.preprocess(
      (value) => {
        const normalized = normalizeString(value);
    ...
    min_price: optionalPositiveNumber({ min: 0, max: PRICE_MAX }),
    max_price: optionalPositiveNumber({ min: 0, max: PRICE_MAX }),
    year_min: optionalPositiveInt({ min: YEAR_MIN, max: YEAR_MAX }),
    year_max: optionalPositiveInt({ min: YEAR_MIN, max: YEAR_MAX }),
    mileage_max: optionalPositiveInt({ min: 0, max: MILEAGE_MAX }),
    body_type: optionalTrimmedString({ min: 1, max: BODY_TYPE_MAX_LENGTH }),
    fuel_type: optionalTrimmedString({ min: 1, max: FUEL_TYPE_MAX_LENGTH }),
    transmission: optionalTrimmedString({ min: 1, max: TRANSMISSION_MAX_LENGTH }),
    below_fipe: optionalBoolean(),
    highlight_only: optionalBoolean(),
    sort: optionalTrimmedString({ min: 1, max: SORT_MAX_LENGTH, toLowerCase: true }),
    page: optionalPositiveInt({ min: 1, max: PAGE_MAX }),
    limit: optionalPositiveInt({ min: 1, max: LIMIT_MAX }),
  })
