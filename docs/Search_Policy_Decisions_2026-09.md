...
**Alvos por perfil.** Ajusta DEC-17 nos perfis de produto específico, mantendo a gradação **entre eles** e o caráter transitório: SEARCH_MODEL 24 · SEARCH_MODEL_YEAR 16 · SEARCH_VERSION 12. Entre famílias, a gradação da v3 §5 — "quanto maior a especificidade, menor o alvo" — deixa de valer: SEARCH_MODEL 24 fica acima de BROWSE_CITY 20 e de SEARCH_BRAND 16, deliberadamente, porque procurar um modelo em catálogo pobre exige território maior do que navegar uma cidade. Esta decisão delimita aquela regra nesse ponto, enquanto durar a fase de formação de estoque. BROWSE_CITY 20, BROWSE_CATEGORY 16 e SEARCH_BRAND 16 permanecem como estão. Continuam sendo configuração (`platform_settings`), não constante, e o gatilho de revisão de DEC-17 continua valendo — com catálogo pobre, alvos desta ordem levam a busca de modelo ao teto automático com frequência, e isso é deliberado enquanto durar a fase de formação de estoque.

**Apresentação.** Quando o raio efetivo é maior que zero, a página declara o raio (DEC-03) e cada resultado exibe a distância até a origem. A ordenação permanece a de DEC-07 — peso comercial primeiro, distância entre anúncios de mesmo peso, ressalvado o teto de participação que DEC-29 introduz dentro da faixa — em **lista única**: separar os resultados da cidade de origem dos das vizinhas rebaixaria um Destaque vizinho para baixo de um anúncio grátis local, o que DEC-07 proíbe. Resultados obtidos por concessão aceita pelo usuário podem aparecer em bloco próprio e rotulado, porque aí a ampliação foi pedida.
...
· Supera a última frase de DEC-18, preservando o restante dela; reafirma DEC-23 quanto ao raio declarado; ajusta DEC-17 nos três perfis citados. Delimita DEC-04 e `V3-INV-005`: quando o baseline de descoberta é maior que o território do perfil, o automático fica com o baseline, e "menor território necessário" passa a valer **a partir** desse piso. Delimita DEC-20 no mesmo ponto que DEC-29: o `effective_radius_km` é o menor anel que contém o conjunto final, não o produto da conversão de `required_distance_km`. Não altera DEC-03, DEC-05, DEC-07, DEC-10, DEC-11, DEC-24 nem DEC-26. · Vigente · 2026-09.
...
**Teto de participação por cidade.** **Dentro de uma mesma faixa de peso comercial**, nenhuma cidade **diferente da cidade de origem** ocupa mais que **40%** da **capacidade daquela faixa na página**. A capacidade é o menor entre o tamanho da página e o número de candidatos daquela faixa no CandidateScope, nunca menor que 1: numa página de 24 vagas cuja faixa Destaque tem 10 candidatos, o teto de uma cidade externa naquela faixa é 4. A base é a faixa, não a página inteira, e não são as vagas que a faixa acabou ocupando — essas dependem da ordenação, que depende do teto, e mediria o resultado contra ele mesmo. O excedente é adiado para as páginas seguintes, de forma determinística, preservando a ordem relativa entre todos os demais anúncios. O teto é regra de **distribuição**, não de truncamento: o adiamento só ocorre quando existem candidatos de outra cidade **na mesma faixa** para ocupar as vagas liberadas. Sem alternativa, o excedente preenche a própria página — uma cidade pequena com um vizinho grande continua entregando a página cheia, em vez de ficar curta por causa do limite. A cidade de origem **não** é limitada: o teto existe para impedir que uma praça externa domine a página territorial, não para rebaixar o estoque da própria cidade.

**Relação com DEC-07.** O teto **não** é exceção à ordenação por peso. Ele age **depois** do peso comercial e antes da distância: diversifica dentro de cada faixa, e entre faixas quem manda é o peso, sempre. Usá-lo como chave primária faria um anúncio grátis da origem subir acima de um Destaque pago da vizinha — exatamente o que DEC-07 proíbe, e o que destruiria a razão de alguém pagar por Destaque numa praça vizinha. O teto nunca limita a origem e nunca remove ninguém do conjunto.

Dentro da faixa, porém, o teto **delimita** a cláusula de distância de DEC-07 e o `V3-INV-047`: quando uma cidade externa já ocupou o seu limite naquela faixa, o próximo anúncio dela é adiado e um anúncio mais distante de outra cidade, do mesmo peso, ocupa a vaga. É reordenação por cidade dentro da faixa, assumida aqui por escrito, e é o preço de existir diversidade territorial. "Distância só ordena entre anúncios de mesmo peso" passa a valer ressalvado esse adiamento; a precedência do peso comercial continua intacta.

**Consequência aceita.** Como o teto só age dentro da faixa, uma cidade externa que domine uma faixa inteira sem concorrente da mesma faixa **ultrapassa** os 40% da página. O teto distribui o que há para distribuir; não fabrica diversidade que o estoque não tem, e não rebaixa anúncio pago para consegui-la. Se a dominância de uma praça vizinha vier a ser um problema real, a correção é de produto — preço, captação ou limite por faixa —, não a inversão da ordem de peso.
...
**Raio declarado.** O piso não altera a regra de DEC-28: `effective_radius_km` descreve o conjunto final de candidatos, não o território consultado. Uma página cujo resultado esteja todo na própria cidade declara 0 km mesmo tendo consultado 25 km. O copy acompanha o declarado, nunca o consultado: nessa página o texto nomeia apenas a cidade e não promete "e região", porque não há vizinho nenhum para mostrar.

**Concessão de distância com piso.** Em Guided Relaxation, o "território atual" contra o qual DEC-26 mede a menor concessão útil e classifica a banda de custo é o território **consultado**, incluído o piso — nunca o raio declarado. Com piso de 25 km e declarado 0 km, uma concessão a 30 km é acréscimo de 5 km, banda `PEQUENA`: tudo abaixo de 25 km já fora consultado e nada havia ali. Isto delimita DEC-26 num ponto que ela não poderia prever, porque antes do piso os dois números coincidiam. Onde a página exibir contagem — inclusive em metadados de SEO —, o número exibido deve ser o do mesmo conjunto que a página mostra.
