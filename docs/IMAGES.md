...
Regra do usuário: *"Não alterar `AdCard` visualmente neste PR, exceto se for estritamente necessário"*. Como a integração não é estritamente necessária (o componente é independente e tem suite de testes própria), fica para **PR F**. Lá, `AdCard` é refatorado em variantes e passa a usar `<VehicleImage>` em todas elas.
