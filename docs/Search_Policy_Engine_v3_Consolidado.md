...
**Delta DEC-28/DEC-29:** NÃO CERTIFICADO — sincronizado em 2026-09-24, corrigido em 2026-09-24 após pré-auditoria documental e endurecido em 2026-09-24 após a auditoria air-gap que o reprovou (território: seções 4, 5, 6, 7, 8, 12, 17, 18 e 23.a, invariantes `V3-INV-008`, `V3-INV-009`, `V3-INV-039`, `V3-INV-047` e `V3-INV-079`…`V3-INV-089`). É conteúdo normativo posterior ao delta certificado em `deb928bc` e, pela governança registrada abaixo, constitui delta não certificado até que uma auditoria documental própria o cubra. Não altera as certificações MET-01, DEC-26 e DEC-27, que continuam valendo apenas para os baselines que certificaram.
...
- **distância** — a menor distância real que acrescenta resultados acima do território atual, arredondada para cima ao quantum configurado e limitada a 150 km; _Conflito: DEC-21 formulava esta dimensão como "o primeiro anel permitido acima do território atual que acrescenta resultados". DEC-26 a supera: a concessão territorial não depende de presets de `rings_manual` e não existe lista normativa obrigatória de anéis de concessão. Vale DEC-26._
...
O pipeline de ranking deve ser, nesta ordem: status e disponibilidade; correspondência com o produto; elegibilidade territorial; peso comercial decrescente; distância crescente; rotação e diversidade (quando ativadas — ADI-02); qualidade e recência; desempate determinístico final.
...
O texto da página deve refletir a realidade do conjunto exibido. Em `EXACT_CITY`, o texto nomeia apenas a cidade e não deve dizer "e região". Com raio declarado maior que 0, o texto deve declarar o raio — obrigação de DEC-03 — e, em busca de produto, deve dizer em até quantos quilômetros de qual origem os resultados foram encontrados. O texto pode informar quantos resultados foram encontrados, desde que a contagem reflita o conjunto efetivamente exibido.
...
| V3-INV-066 | A relaxação de distância usa a menor distância real que acrescenta resultados acima do território atual, arredondada para cima ao quantum e limitada a 150 km, aplicada como raio manual e nunca reintroduzida no automático                                                                                                         | DEC-21 · DEC-26            |
