...
_Conflito: a v2.0 §44 lista um único vocabulário de contexto — consulta, modelo comercial, especificidade, cidade de origem, fonte da localização, modo geográfico, raio solicitado, raio efetivo, contagem de resultados, contagem de vendedores, peso comercial, distância, posição e versão de política — compartilhado por eventos de granularidades diferentes, cinco deles sobre um resultado específico (resultados servidos, clique no resultado, visualização do veículo, lead criado, clique de WhatsApp). Esta seção havia transportado a lista integralmente para `search.executed`, que é por busca, sem que nenhuma fonte dissesse de qual resultado extrair peso comercial, distância e posição. Vale DEC-27: esses três campos pertencem a eventos result-scoped e não são obrigatórios em `search.executed`. A correção é de granularidade, não de alcance: nenhuma grandeza de busca sai do payload._
...
**Contagem de vendedores** é o número de vendedores distintos no mesmo `CandidateScope` e no mesmo território efetivo usados pela contagem de resultados, antes da paginação — mesmos filtros, mesmo SearchContext, mesma elegibilidade, sem ponderação comercial. Não conta apenas os vendedores da página exibida e não é derivada da Guided Relaxation.

**Campos por resultado.** Peso comercial, distância e posição pertencem a eventos que identificam um resultado específico — os eventos por resultado da v2.0 §44 ou outro evento futuro explicitamente result-scoped —, adiados por ADI-01. Quando existirem: a posição é a posição 1-based daquele resultado na lista efetivamente apresentada ao usuário; o peso comercial é o peso efetivo daquele anúncio naquele SearchContext, conforme a regra de ranking vigente (seção 12); a distância é a daquele anúncio em relação à origem efetiva da busca, conforme a semântica geográfica vigente (`V3-INV-034`). Em `search.executed`, nenhum dos três é substituído por média, mediana, máximo, mínimo ou valor do primeiro resultado sem nova decisão normativa, e nenhuma posição 1 artificial é emitida por se ter escolhido o primeiro resultado.

O shadow controlado não depende desses eventos: pode operar com o contexto de busca, as contagens, os identificadores de primeiro resultado já usados na comparação e o status de timeout ou divergência.
...
| V3-INV-052 | Na fase atual, um único evento `search.executed`, com payload completo de contexto da busca, é suficiente. Seu mínimo obrigatório é: consulta, modelo comercial, especificidade, cidade de origem, fonte da localização, modo geográfico, raio solicitado, raio efetivo, contagem de resultados, contagem de vendedores e versão de política. Campos intrinsecamente ligados a um resultado — peso comercial, distância e posição — pertencem a eventos result-scoped posteriores e não são obrigatórios em `search.executed`. | ADI-01 · DEC-27            |
...
**Contagem.** IDs `V3-INV` alocados: **78**. Retirados: **3** — `V3-INV-029`, `V3-INV-031` e `V3-INV-040`. Invariantes normativos ativos: **75**. DEC-26 **não** criou invariante novo: refinou o enunciado e a proveniência de `V3-INV-012`, `V3-INV-013`, `V3-INV-061`, `V3-INV-062`, `V3-INV-063` e `V3-INV-066`, que continuam representando as mesmas obrigações sob os mesmos IDs, e por isso o total de ativos permanece **75**. DEC-27 também **não** criou invariante novo: reformulou o enunciado e a proveniência de `V3-INV-052`, que continua representando a mesma obrigação — o evento único de telemetria e o seu mínimo — sob o mesmo ID; o total de ativos continua **75**. Um ID retirado permanece reservado para rastreabilidade e nunca é reutilizado por outro requisito, do mesmo modo que a lacuna deliberada do DEC-16 no registro de decisões.
