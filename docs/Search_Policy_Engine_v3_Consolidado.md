...
**Delta DEC-28/DEC-29:** NÃO CERTIFICADO — sincronizado em 2026-09-24 (território: seções 6, 7, 8 e 12, invariantes `V3-INV-009`, `V3-INV-016` e `V3-INV-079`…`V3-INV-086`). É conteúdo normativo posterior ao delta certificado em `deb928bc` e, pela governança registrada abaixo, constitui delta não certificado até que uma auditoria documental própria o cubra. Não altera as certificações MET-01, DEC-26 e DEC-27, que continuam valendo apenas para os baselines que certificaram.
...
A rota `/carros-em/[cidade]` deve nascer no perfil `BROWSE_CITY` em modo `AUTO_RADIUS`. O automático escolhe o menor território que atinge liquidez útil, partindo do piso regional (seção 6): com estoque próprio suficiente o território fica no piso; caso contrário, expande para 50 ou 75 km, parando no primeiro anel que atinge o alvo.

_Refina DEC-03 por DEC-29: o resultado "raio efetivo 0 km" deixa de ser possível em modo automático, porque o piso é mínimo. O 0 km continua existindo como escolha manual e como raio **declarado** quando o conjunto final está todo na própria cidade._
...
| V3-INV-016 | Na entrada direta em `/comprar`, com origem conhecida e sem raio ou escopo manual, o território final nunca é menor que o território-base de descoberta                                                                                                                                                                              | DEC-18 · DEC-28            |
...
| V3-INV-083 | Nenhuma cidade diferente da origem ocupa mais que o teto de participação de uma página                                                                                                                                                                                                                                               | DEC-29                     |
| V3-INV-084 | O excedente do teto só é adiado quando há candidato de outra cidade para ocupar a vaga                                                                                                                                                                                                                                               | DEC-29                     |
...
**Contagem.** IDs `V3-INV` alocados: **86**. Retirados: **3** — `V3-INV-029`, `V3-INV-031` e `V3-INV-040`. Invariantes normativos ativos: **83**. DEC-26 **não** criou invariante novo: refinou o enunciado e a proveniência de `V3-INV-012`, `V3-INV-013`, `V3-INV-061`, `V3-INV-062`, `V3-INV-063` e `V3-INV-066`, que continuam representando as mesmas obrigações sob os mesmos IDs, e por isso o total de ativos permanece **75**. DEC-27 também **não** criou invariante novo. `V3-INV-052` preserva o mesmo ID porque continua governando o contrato mínimo do único evento obrigatório de telemetria; DEC-27 corrige quais grandezas pertencem semanticamente ao nível da busca, reduzindo esse mínimo de 14 para 11 grandezas, e reformula o enunciado e a proveniência do invariante. O total de ativos permanece **75**. DEC-28 e DEC-29 criam oito invariantes novos — `V3-INV-079` a `V3-INV-086` — e reformulam `V3-INV-009` e `V3-INV-016`, que preservam seus IDs porque continuam governando as mesmas obrigações, agora com o baseline como piso. Com eles, os IDs alocados passam a **86** e os ativos a **83**, mantidos os três retirados. Um ID retirado permanece reservado para rastreabilidade e nunca é reutilizado por outro requisito, do mesmo modo que a lacuna deliberada do DEC-16 no registro de decisões.
