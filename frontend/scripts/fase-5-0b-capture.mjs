  ...
  return page.evaluate(() => {
    ...
    return {
      ...
      containerWidth: Math.round(
        (grid.closest("div.mx-auto") ?? document.body).getBoundingClientRect().width
      ),
    };
  });
}
...
const VIEWPORTS = [
  ...
  { file: "02-city-desktop-1440", w: 1440, h: 900, full: false },
  { file: "03-city-desktop-1920-4-colunas", w: 1920, h: 1080, full: false },
  { file: "04-city-desktop-1280-fullpage", w: 1280, h: 900, full: true },
  { file: "05-city-desktop-1920-fullpage", w: 1920, h: 1080, full: true },
  { file: "06-city-mobile-390", w: 390, h: 844, full: false },
  { file: "07-city-mobile-390-fullpage", w: 390, h: 844, full: true },
  { file: "08-city-tablet-768", w: 768, h: 1024, full: false },
];

const BASELINE_ONLY = new Set([
  "01-city-desktop-1280",
  "04-city-desktop-1280-fullpage",
  "06-city-mobile-390",
  "07-city-mobile-390-fullpage",
]);
...
for (const vp of VIEWPORTS) {
  ...
  console.log(
    `[${label}] ${vp.file} status=${status} perRow=${m?.perRow} card=${m?.cardWidth}x${m?.cardHeight} doc=${m?.docHeight}px`
  );
  await ctx.close();
}
...
if (label === "branch") {
  for (const [file, w, h] of [
    ["09-city-paginacao-desktop", 1440, 900],
    ["10-city-footer-transition", 1440, 900],
  ]) {
    ...
    if (file === "09-city-paginacao-desktop") {
