...
function buildSuggestedTitle(form: FormState) {
  return [form.yearModel, form.brand, form.model, form.version]
    .filter(Boolean)
    .join(" ")
    .trim();
}
...
function InputField({
  ...
  return (
    <label className="block">
      <span className="mb-2 block text-sm font-semibold text-[#1D2440]">
        {label}
      </span>
      <input
        type={type}
...
export default function NewAdPageClient({ initialType }: Props) {
  ...
  useEffect(() => {
    try {
      ...
      setForm((prev) => ({
        ...
        ...parsed,
        sellerType:
          parsed.sellerType === "lojista" ? "lojista" : prev.sellerType,
      }));
          ...
          <div className="space-y-6">
            ...
            >
              <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
                <InputField label="Marca" value={form.brand} onChange={(value) => updateField("brand", value)} placeholder="Ex.: Toyota" />
                <InputField label="Modelo" value={form.model} onChange={(value) => updateField("model", value)} placeholder="Ex.: Corolla" />
                <InputField label="Versão" value={form.version} onChange={(value) => updateField("version", value)} placeholder="Ex.: XEi 2.0 Flex Automático" />
                <InputField label="Ano/modelo" value={form.yearModel} onChange={(value) => updateField("yearModel", value)} placeholder="Ex.: 2021/2022" />
                <InputField label="Cidade" value={form.city} onChange={(value) => updateField("city", value)} placeholder="Ex.: São Paulo" />
                <InputField label="Estado" value={form.state} onChange={(value) => updateField("state", value.toUpperCase())} placeholder="SP" />
                <InputField label="Título do anúncio" value={form.title} onChange={(value) => updateField("title", value)} placeholder={suggestedTitle || "Será sugerido automaticamente"} />
            ...
            >
              <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
                <InputField label="Preço do anúncio" value={form.price} onChange={handlePriceChange} placeholder="R$ 0,00" />
                <InputField label="Valor FIPE" value={form.fipeValue} onChange={handleFipeChange} placeholder="R$ 0,00" />
                <InputField label="Quilometragem" value={form.mileage} onChange={(value) => updateField("mileage", value.replace(/[^\d]/g, ""))} placeholder="Ex.: 32500" />
                <InputField label="Final de placa" value={form.plateFinal} onChange={(value) => updateField("plateFinal", value.replace(/[^\d]/g, ""))} placeholder="Ex.: 7" />

                <SelectField label="Combustível" value={form.fuel} onChange={(value) => updateField("fuel", value)} options={["Flex", "Gasolina", "Diesel", "Híbrido", "Elétrico"]} />
                <SelectField label="Câmbio" value={form.transmission} onChange={(value) => updateField("transmission", value)} options={["Automático", "Manual", "CVT", "Automatizado"]} />
                <SelectField label="Carroceria" value={form.bodyStyle} onChange={(value) => updateField("bodyStyle", value)} options={["Sedã", "Hatch", "SUV", "Picape", "Cupê", "Van"]} />
                <InputField label="Cor" value={form.color} onChange={(value) => updateField("color", value)} placeholder="Ex.: Prata" />
              </div>
              ...
              <div className="mt-5 grid gap-4 lg:grid-cols-3">
                ...
                <div className="rounded-[24px] border border-[#E5E9F2] bg-white p-5">
                  ...
                  <div className={`mt-2 text-[18px] font-bold ${fipeNumber > 0 ? (belowFipe ? "text-green-600" : "text-orange-600") : "text-[#1D2440]"}`}>
                    {fipeNumber > 0
              ...
              {photoPreviews.length ? (
                <div className="mt-5 grid gap-4 sm:grid-cols-2 xl:grid-cols-5">
                  {photoPreviews.map((src, index) => (
                    <div key={`${src}-${index}`} className="overflow-hidden rounded-[22px] border border-[#E5E9F2] bg-white">
            ...
            >
              <div className="grid gap-4 md:grid-cols-2">
                <InputField label="WhatsApp" value={form.whatsapp} onChange={(value) => updateField("whatsapp", value)} placeholder="(11) 99999-9999" />
                <InputField label="Telefone" value={form.phone} onChange={(value) => updateField("phone", value)} placeholder="(11) 3333-3333" />
              </div>
            ...
            <SectionCard title="Ações do anúncio" subtitle="Você pode salvar, revisar e publicar para o endpoint configurado.">
              ...
              <div className="mt-4 text-sm">
                {saveState === "saved" && <span className="text-[#15803D]">Rascunho salvo no navegador com sucesso.</span>}
                {saveState === "loaded" && <span className="text-[#1F66E5]">Rascunho anterior carregado automaticamente.</span>}
                {saveState === "idle" && <span className="text-[#6E748A]">Nenhuma ação recente de rascunho.</span>}
              </div>
            ...
            {showReview ? (
              <SectionCard title="Revisão do anúncio" subtitle="Prévia textual do anúncio pronta para publicação.">
                <div className="grid gap-6 lg:grid-cols-[320px_minmax(0,1fr)]">
                  <div className="overflow-hidden rounded-[24px] border border-[#E5E9F2] bg-white">
                    <div className="aspect-[4/3] bg-[#EDF2FB]">
                      {photoPreviews[0] ? (
                        <img src={photoPreviews[0]} alt="Prévia do anúncio" className="h-full w-full object-cover" />
                      ) : (
                      ...
                      <div key={item.label} className="flex items-center gap-3">
                        <span
                          className={`inline-flex h-6 w-6 items-center justify-center rounded-full text-xs font-bold ${
                            item.ok ? "bg-[#EAF8EF] text-[#15803D]" : "bg-[#EFF3F9] text-[#6E748A]"
                          }`}
                        >
                          {item.ok ? "✓" : "•"}
                        </span>
