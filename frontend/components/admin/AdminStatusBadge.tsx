...
export function AdminStatusBadge({ status }: { status: string }) {
  const s = MAP[status?.toLowerCase()] ?? { bg: "bg-gray-200", text: "text-gray-600", label: status || "—" };
  return (
    <span className={`inline-block rounded-full px-2.5 py-0.5 text-[11px] font-bold ${s.bg} ${s.text}`}>
      {s.label}
    </span>
  );
}
