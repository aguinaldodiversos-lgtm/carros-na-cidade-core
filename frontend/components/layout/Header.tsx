...
export default function Header({ boxed = false }: HeaderProps) {
  ...
  return (
    <header
      className={`${boxed ? "pt-3 md:pt-6" : "border-b border-[#e4e7f0] bg-white"} sticky top-0 z-40`}
    >
      <div className="mx-auto w-full max-w-7xl px-3 sm:px-4 md:px-6">
        <div
          className={`flex items-center justify-between px-2 ${
            boxed
              ? "h-[74px] rounded-2xl border border-[#e4e7f0] bg-white md:h-[86px]"
              : "h-[70px] md:h-[82px]"
          }`}
        >
          <div className="flex min-w-0 items-center gap-2 sm:gap-5">
            ...
            <button
              type="button"
              className="hidden items-center gap-2 text-[16px] font-bold text-[#20273a] lg:inline-flex"
            >
              Sao Paulo
          ...
          <nav className="hidden items-center gap-6 text-[16px] font-semibold text-[#333d54] md:flex">
            ...
            <Link
              ...
              className="text-[#4f5a74] transition hover:text-[#0e62d8]"
            >
              <HeartIcon />
            </Link>
            <Link
