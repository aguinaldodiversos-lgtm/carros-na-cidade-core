...
export async function GET(request: NextRequest) {
  ...
  if (!url) {
    return NextResponse.json({ success: false, message: "URL inválida.", data: [] }, { status: 500 });
  }
  ...
  try {
    ...
    return NextResponse.json({ success: false, message: "Resposta inválida.", data: [] }, { status: 502 });
  }
