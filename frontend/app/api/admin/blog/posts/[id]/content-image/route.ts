...
/**
 * BFF de upload de imagem no MEIO do conteúdo do post (Fase 4.2.2).
 *
 * Pass-through multipart para POST /api/admin/blog/posts/:id/content-image.
 * Rota dedicada (igual à da capa) porque o proxy genérico [...path] lê o body
 * com request.text() e corromperia o binário — aqui usamos arrayBuffer.
 *
 * Não dispara revalidate: o upload sozinho não muda nada público; a imagem só
 * entra no post quando o admin salva (PATCH content com ![alt](url)).
 */
...
export async function POST(request: NextRequest, { params }: { params: { id: string } }) {
  ...
  const backendUrl = resolveInternalBackendApiUrl(`/api/admin/blog/posts/${id}/content-image`);
