...
/**
 * BFF dedicado para o hero da Home (Fase 4.1).
 *
 * Por que rota dedicada em vez do proxy `[...path]` genérico?
 * -----------------------------------------------------------
 * Após PATCH /api/admin/home/hero, precisamos invalidar o cache do Next
 * (tag `public-home-hero` + path `/`). O proxy genérico
 * `app/api/admin/[...path]/route.ts` não conhece esta semântica. Em vez
 * de poluí-lo com lógica específica, esta rota mais específica tem
 * prioridade no Next routing e implementa: proxy ao backend + revalidate
 * em caso de sucesso.
 *
 * GET: passa straight para o backend (sem revalidate).
 * PATCH: proxy + se 2xx → revalidate tag + path (best-effort, não falha
 *        o PATCH se revalidate falhar).
 */
...
async function proxy(request: NextRequest) {
  ...
  const backendUrl = resolveInternalBackendApiUrl(`/api/admin/home/hero${search}`);
  ...
  try {
    ...
    // PATCH bem-sucedido → dispara revalidate do cache público.
    // Best-effort: erro aqui NÃO falha a resposta ao admin.
    if (request.method === "PATCH" && upstream.ok) {
      try {
        await triggerHomeHeroRevalidate();
      } catch (err) {
        console.warn("[admin/home/hero] revalidate falhou", {
