...
export const metadata: Metadata = { title: "Leads | Dashboard Loja" };
export const dynamic = "force-dynamic";

export default async function DashboardLojaLeadsPage() {
  const cookieStore = cookies();
  const session = getSessionDataFromCookieValue(cookieStore.get(AUTH_COOKIE_NAME)?.value);
  if (!session?.id || !session.accessToken) redirect("/login?next=%2F/dashboard-loja%2Fleads");
  return (
    <main className="mx-auto w-full max-w-7xl px-4 py-6 sm:px-6">
      <DashboardLojaNav />
      <div className="mt-6">
        <h1 className="text-[24px] font-extrabold text-[#1d2538]">Leads</h1>
