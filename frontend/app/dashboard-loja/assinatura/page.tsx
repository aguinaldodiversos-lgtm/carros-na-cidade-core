...
export const metadata: Metadata = { title: "Assinatura | Dashboard Loja" };
export const dynamic = "force-dynamic";

export default async function DashboardLojaAssinaturaPage() {
  const cookieStore = cookies();
  const session = getSessionDataFromCookieValue(cookieStore.get(AUTH_COOKIE_NAME)?.value);
  if (!session?.id || !session.accessToken) redirect("/login?next=%2F/dashboard-loja%2Fassinatura");
  return (
