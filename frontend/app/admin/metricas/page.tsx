...
export default function AdminMetricas() {
  const topAds = useAdminFetch<{ ok: boolean; data: TopAdMetric[] }>(
    () => adminApi.metrics.topAds(15),
    []
  );
  const cities = useAdminFetch<{ ok: boolean; data: CityMetric[] }>(
    () => adminApi.metrics.cities(20),
    []
  );
  const events = useAdminFetch<{ ok: boolean; data: RecentEvent[] }>(
    () => adminApi.metrics.recentEvents(30),
    []
  );
  const seo = useAdminFetch<{ ok: boolean; data: SeoCityMetric[] }>(
    () => adminApi.metrics.seoCities(20),
    []
  );
            ...
            <table className="w-full text-left text-xs">
              <thead>
                <tr className="bg-cnc-bg/50 border-t border-cnc-line">
                  ...
                  <th className="px-4 py-2 font-semibold text-cnc-muted uppercase tracking-wider text-right">
                    Views
                  </th>
                  <th className="px-4 py-2 font-semibold text-cnc-muted uppercase tracking-wider text-right">
                    Clicks
              ...
              <tbody>
                {(topAds.data?.data ?? []).map((ad) => (
                  ...
                  >
                    ...
                    <td className="px-4 py-2.5 font-medium text-cnc-text max-w-[200px] truncate">
                      {ad.title}
                    </td>
                    <td className="px-4 py-2.5 text-cnc-muted">
                      {ad.city}/{ad.state}
                    </td>
                    <td className="px-4 py-2.5 text-right font-semibold text-cnc-text">
              ...
              <tbody>
                {(cities.data?.data ?? []).map((c) => (
                  ...
                  >
                    ...
                    <td className="px-4 py-2.5 text-right text-cnc-text">
                    ...
                    <td className="px-4 py-2.5 text-right text-cnc-text">
                      {c.ads_count.toLocaleString("pt-BR")}
                    </td>
              ...
              <tbody>
                {(events.data?.data ?? []).map((e) => (
                  ...
                  >
                    ...
                    <td className="px-4 py-2.5 font-medium text-cnc-text max-w-[200px] truncate">
                      {e.ad_title || `Anúncio #${e.ad_id}`}
                    </td>
                    <td className="px-4 py-2.5">
                      <span className="rounded-full bg-cnc-bg px-2 py-0.5 text-[11px] font-semibold text-cnc-text">
                        {e.event_type}
                      </span>
                    </td>
