...
export default function AdminDashboard() {
  ...
  const paysRes = useAdminFetch(
    () => adminApi.payments.list({ limit: 5 }),
    []
  );
  ...
  if (hasError) {
    return <AdminErrorState message={overview.error || kpis.error || "Erro"} onRetry={() => { overview.reload(); kpis.reload(); }} />;
  }
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
        <AdminKpiCard label="Anúncios Ativos" value={fmt(ov?.ads?.active)} subtitle={`${fmt(ov?.ads?.total)} total`} color="#1a56db" />
        <AdminKpiCard label="Anunciantes Ativos" value={fmt(ov?.advertisers?.active)} subtitle={`${fmt(ov?.advertisers?.total)} total`} color="#0f9f6e" />
        <AdminKpiCard label="Custo da Campanha" value={money(kp?.revenue?.boost_revenue)} subtitle={`${fmt(kp?.new_ads)} novos anúncios (30d)`} color="#d18a12" />
        <AdminKpiCard label="Faturamento Total" value={money(kp?.revenue?.total_approved)} subtitle={`Planos: ${money(kp?.revenue?.plan_revenue)}`} color="#7c3aed" />
      </div>
      ...
      <div className="grid grid-cols-1 gap-4 lg:grid-cols-2">
        <AdminMiniBarChart
          ...
          items={topCities.slice(0, 6).map((c) => ({
            ...
            values: [
              { name: "Anúncios", value: c.ads_count, color: "#1a56db" },
            ],
          }))}
        />
        <AdminChartCard
      ...
      <div className="grid grid-cols-1 gap-4 lg:grid-cols-2">
        <AdminTableCard
          ...
          columns={[
            { key: "title", label: "Título", render: (r) => <span className="font-medium truncate max-w-[180px] block">{String(r.title)}</span> },
            { key: "city", label: "Cidade", render: (r) => <span>{String(r.city)}/{String(r.state)}</span> },
            { key: "status", label: "Status", render: (r) => <AdminStatusBadge status={String(r.status)} /> },
            { key: "created_at", label: "Data", render: (r) => <span>{shortDate(String(r.created_at))}</span> },
          ]}
        ...
        <AdminTableCard
          ...
          columns={[
            { key: "user_name", label: "Usuário", render: (r) => <span className="font-medium">{String(r.user_name || r.user_email || "—")}</span> },
            { key: "amount", label: "Valor", render: (r) => <span className="font-semibold">{money(Number(r.amount))}</span> },
            { key: "context", label: "Contexto", render: (r) => <span className="capitalize">{String(r.context)}</span> },
            { key: "status", label: "Status", render: (r) => <AdminStatusBadge status={String(r.status)} /> },
          ]}
        ...
        <AdminTableCard
          title="Top Cidades"
          columns={[
            { key: "name", label: "Cidade", render: (r) => <span className="font-medium">{String(r.name)}/{String(r.state)}</span> },
            { key: "ads_count", label: "Anúncios", render: (r) => <span className="font-semibold">{fmt(Number(r.ads_count))}</span> },
          ]}
